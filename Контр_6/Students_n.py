class Student:
    def __init__(self, surname, first_name, second_name,
                 birthday, speciality, year, group, entry_points, avg_points):
    ...
    def get_info(self):
        ...
        return [name, self.birthday,self.speciality,self.year,self.group,self.entry_points,self.avg_points]
    ...
    def load_students_list(file_name, speciality):
        ...
        for st in st_list:
            if(st.endswith('\n')):
               ...
               students.append(Student(sur,f_name,s_name,birth,speciality, int(year) , group, int(entry_p), float(avg_p)))
